...
void MySoftmaxWithLossLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
  CHECK_EQ(bottom.size(), 2) << "MySoftmaxLoss Layer takes two blobs as input.";
...
template <typename Dtype>
Dtype MySoftmaxWithLossLayer<Dtype>::Forward_cpu(
  ...
  Dtype loss = 0;
  for (int i = 0; i < num; ++i) {
    loss += -log(max(prob_data[i * dim + labelR[i] ],
                     Dtype(FLT_MIN)));
    int id = labelR[i] / nClassLabel;
    for (int j = 0; j < nRotation; ++j) 
      if( j != id )
	loss += -log(max(prob_data[i * dim + j * nClassLabel + nClassLabel - 1 ],
			 Dtype(FLT_MIN)));
  }
